...
async function completePayment(event) {
    ...
    // Guard against a fast double-click firing this twice — the second
    // call would find the cart already cleared, hit the "cart is empty"
    // alert, and that alert covers the success modal (looks like it
    ...
    try {
        ...
        // Send the completed order to FastAPI
        ...
        if (!response.ok) {
            ...
            console.error("Order rejected by server:", errorBody);
        ...
        // Order is confirmed saved — clear the cart now, so it's empty
        // regardless of whether the user clicks "Close" on the popup
        // or navigates away some other way.
